...
export default function BookingPage() {
  ...
  async function handleSubmit(e: any) {
    e.preventDefault();
    const { data } = await supabase.from('leish_bookings').insert({ mua_id: mua.id, service_id: service.id, booking_date: form.date, booking_time: form.time, total_amount: service.price, client_name: form.name, client_email: form.email, client_phone: form.phone, status: 'pending' }).select().single();
    if (data) {
